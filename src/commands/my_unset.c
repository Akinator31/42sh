...
static bool check_variable_name(const char *name)
{
    int i = 0;
...
static void replace_from_index(char **environ, int start, int end)
{
    int i = start;
...
static int my_unsetenv(char **new_environ, const char *name)
{
    int i = 0;
...
static bool not_enough_args(bool is_correct_cmd, int nb_args,
...
static bool was_unset(char **cmd_args, sh_t *sh_st)
{
...
bool is_unset_command(sh_t *sh_st, exit_status_t *status)
{
    ...
    bool is_unsetenv = was_unset(cmd_args, sh_st);

    if (!not_enough_args(is_correct_cmd, nb_args, cmd_args, sh_st->error_code))
        return is_unsetenv;
    while (is_correct_cmd && cmd_args[i]) {
        new_environ = duplicate_2d_char_array(sh_st->var,
            get_2d_arr_len(sh_st->var) + 1);
        my_unsetenv(new_environ, cmd_args[i]);
        sh_st->var = new_environ;
        i++;
    }
    *status = NORMAL;
    free_2d_array_of_char(cmd_args);
