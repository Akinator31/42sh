...
Test(analyse_command, basic_analyse_command)
{
    ...
    cr_assert_eq(analyse_command(&env, "/bin/ls", &error_code, NULL), NORMAL);
}
...
Test(analyse_command_, basic_analyse_command_pipe)
{
    ...
    cr_assert_eq(analyse_command(&env, "ls | cat -e", &error_code, NULL), NORMAL);
}
...
Test(is_cd_command, is_cd_command_false)
{
    ...
    sh_t sh_st = {&env, "ls", &error_code, NULL};
    if (status == NULL)
        cr_assert_fail("Memory allocation for status failed");
    cr_assert_eq(is_cd_command(&sh_st, status), false);
    free(status);
}
...
Test(is_cd_command, is_cd_command_true)
{
    ...
    int error_code = 1;
    exit_status_t *status = malloc(sizeof(exit_status_t));
    sh_t sh_st = {&env, "cd", &error_code, NULL};
    if (status == NULL)
        cr_assert_fail("Memory allocation for status failed");;
    
    cr_assert_eq(is_cd_command(&sh_st, status), true);
    free(status);
}
...
Test(is_env_command, is_env_command_false)
{
    ...
    cr_assert_eq(is_env_command(&sh_st, status), false);
    free(status);
}
...
Test(is_env_command, is_env_command_true)
{
    ...
    int error_code = 1;
    exit_status_t *status = malloc(sizeof(exit_status_t));
    sh_t sh_st = {&env, "env", &error_code, NULL};
    if (status == NULL)
        cr_assert_fail("Memory allocation for status failed");

    cr_assert_eq(is_env_command(&sh_st, status), true);
    free(status);
}
